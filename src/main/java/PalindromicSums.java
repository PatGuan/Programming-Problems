...
/**
     ...
     Note that 1=02+121=02+12 has not been included as this problem is concerned with the squares of positive integers. Also, there has to be at least two terms in the sum.
...
public class PalindromicSums {
    ...
    public int init(int numericSumLimit, int baseDifference) {
    ...
    private HashSet<Integer> getPalindromicSums(int numericSumLimit, int baseDifference, int startingIteratingPosition) {
        int i;
    ...
    private HashSet<Integer> findPalindromicSumsForInteger(int numericSumLimit, int baseDifference, int startingIteratingPosition) {
        int i;
        HashSet<Integer> palindromicSums = new HashSet<Integer>();
        int currentTotal = 0;
        for (i = startingIteratingPosition; i < Math.sqrt(numericSumLimit); i+=baseDifference) {
            ...
            else if (isNumberPalindrome(currentTotal)) {
    ...
    public boolean isNumberPalindrome(int number) {
        ...
        int i;
        if (stringNumber.length() == 2 && stringNumber.charAt(0) == stringNumber.charAt(1)) {
            return true;
        }
        else if (stringNumber.length() > 2) {
            for (i = 0; i < Math.ceil(new Double(stringNumber.length()/2.0)); i++) {
